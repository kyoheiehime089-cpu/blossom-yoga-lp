-- v35: ヨガ個別予約をセルフ予約不可時間として自動反映する外部ブロック機能
...
create table if not exists public.fs_external_blocks (
  ...
  title text not null default 'ヨガ個別予約',
  member_name text not null,
  instructor_name text,
  note text,
  created_at timestamptz not null default now(),
  created_by text,
  constraint fs_external_blocks_time_check check (
    start_minute >= 0 and start_minute <= 1430 and
    end_minute >= 10 and end_minute <= 1440 and
    end_minute > start_minute and
    start_minute % 10 = 0 and
    end_minute % 10 = 0
  ),
  constraint fs_external_blocks_source_check check (source in ('yoga_private'))
);

alter table public.fs_external_blocks enable row level security;
revoke all on public.fs_external_blocks from anon, authenticated;

create index if not exists fs_external_blocks_date_time_idx
on public.fs_external_blocks(date,start_minute,end_minute)
where source = 'yoga_private';
...
as $$
  select exists(
    ...
    where b.date = p_date
      and public.fs_range_overlaps(p_start_minute,p_end_minute,b.start_minute,b.end_minute)
  );
$$;
...
as $$
declare
  clean_member text := btrim(coalesce(p_member_name,''));
...
begin
  ...
  if public.fs_slot_blocked(p_date,p_start_minute) then
    return jsonb_build_object('ok',false,'error','この時間はすでに予約または利用不可枠と重なっています。');
  end if;
...
create or replace function public.fs_yoga_private_delete(p_admin_password text, p_external_block_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from public.fs_external_blocks
  where id = p_external_block_id
    and source = 'yoga_private';

  if not found then
    return jsonb_build_object('ok',false,'error','ヨガ個別予約が見つかりません。');
  end if;
...
grant execute on function public.fs_yoga_private_delete(text,uuid) to anon, authenticated;
