...
const gyotokuSemiPersonalSlots=()=>[[1110,1150,'セミパーソナル'],[1160,1200,'セミパーソナル'],[1210,1250,'セミパーソナル'],[1260,1300,'セミパーソナル']];
const gyotokuWeekendSlots=()=>[[600,640,'セミパーソナル'],[650,690,'セミパーソナル'],[700,740,'セミパーソナル'],[750,790,'セミパーソナル']];
function gyotokuYogaFixedBlocks(date){
  const day=new Date(`${date}T00:00:00`).getDay();
  if(GYOTOKU_YOGA_HOLIDAYS.includes(date))return[[540,580,'通常ヨガ'],...gyotokuWeekendSlots()];
  if(day===1)return[[540,580,'通常ヨガ'],...gyotokuSemiPersonalSlots()];
  if(day===2){
    const blocks=[[750,790,'通常ヨガ'],...gyotokuSemiPersonalSlots()];
    if(date<'2026-09-01')blocks.unshift([540,580,'通常ヨガ']);
    return blocks;
  }
  if(day===3)return gyotokuSemiPersonalSlots();
  if(day===4)return[[720,760,'通常ヨガ'],[1245,1285,'通常ヨガ']];
  if(day===5)return gyotokuSemiPersonalSlots();
  if(day===6){
    const blocks=[...gyotokuWeekendSlots(),[1020,1060,'通常ヨガ']];
    if(date<'2026-09-01')blocks.unshift([490,530,'通常ヨガ']);
    return blocks;
  }
  if(day===0)return[[490,530,'通常ヨガ'],...gyotokuWeekendSlots()];
  return[];
}
...
function gyotokuYogaDayOffset(baseDate,itemDate){
  const base=new Date(`${baseDate}T00:00:00`),item=new Date(`${String(itemDate).slice(0,10)}T00:00:00`);
  return Math.round((item-base)/86400000)*1440;
}
function gyotokuYogaProjectedBlock(baseDate,item,startMinute,endMinute,reason){
  const offset=gyotokuYogaDayOffset(baseDate,item.date);
  const start=offset+Number(startMinute),end=offset+Number(endMinute);
  if(end<=0||start>=1440)return null;
  return{start_minute:Math.max(0,start),end_minute:Math.min(1440,end),reason};
}
function gyotokuYogaUnavailableBlocks(date){
  ...
  const gym=(gyotokuYogaSnapshot.gym_reservations||[]).map(r=>gyotokuYogaProjectedBlock(date,r,Number(r.start_minute)-30,Number(r.start_minute)+Number(r.use_minutes||40)+30,'行徳ジム24予約（前後30分を含む）')).filter(Boolean);
  const closed=(gyotokuYogaSnapshot.closed_slots||[]).map(r=>gyotokuYogaProjectedBlock(date,r,Number(r.start_minute)-30,Number(r.start_minute)+Number(r.block_minutes||50)+30,r.reason?`利用不可枠（${r.reason}）`:'利用不可枠')).filter(Boolean);
  const privateYoga=(gyotokuYogaSnapshot.yoga_reservations||[]).map(r=>gyotokuYogaProjectedBlock(date,r,Number(r.start_minute),Number(r.end_minute),'ヨガ個別予約')).filter(Boolean);
...
async function gyotokuYogaTimes(preferred=''){
  ...
  const starts=(d.starts||[]).map(Number);
...
function gyotokuYogaRender(){
  const l=document.querySelector('#yogaList');
  const reservations=[...(gyotokuYogaSnapshot.yoga_reservations||[])].sort((a,b)=>{
    const dateCompare=String(b.date||'').localeCompare(String(a.date||''));
    return dateCompare!==0?dateCompare:Number(b.start_minute||0)-Number(a.start_minute||0);
  });
  l.innerHTML=reservations.length?reservations.map(r=>`<article class="res"><h3>ヨガ個別予約</h3><p>${String(r.date).slice(0,10)} ${gyotokuYogaTime(r.start_minute)}〜${gyotokuYogaTime(r.end_minute)}</p><p>会員名：${gyotokuYogaEsc(r.member_name)}</p><button class="danger" data-central-yoga-delete="${r.id}">削除</button></article>`).join(''):'<article class="res"><p>登録済み予約はありません。</p></article>';
  gyotokuYogaRenderUnavailable();
}
