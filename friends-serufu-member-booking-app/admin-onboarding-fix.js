document.addEventListener('DOMContentLoaded',()=>{
  ...
  function normalizePlanFix(p){if(p==='ファミリー週1（月4回）')return'ファミリー月4回プラン';if(p==='ファミリー週2（月8回）')return'ファミリー月8回プラン';return p||'月4回プラン'}
  ...
  form.addEventListener('submit',e=>{
    ...
    const m={id:nextIdFix(),pin:pinFix(),name,email,plan,extraSlots:0};
    S.members.push(m);
    save();
    form.reset();
