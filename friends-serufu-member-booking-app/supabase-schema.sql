-- friendsセルフ予約アプリ Supabase 初期SQL
-- Supabase > SQL Editor にこのSQLを貼り付けて実行してください。
-- 実行後、app_settings の admin_password は必ず変更してください。
...
create unique index if not exists reservations_one_active_slot
on reservations(date,start_minute)
...
revoke all on app_settings, members, reservations, closed_slots, slot_purchases from anon, authenticated;
...
as $$
  select exists(select 1 from app_settings where key='admin_password' and value = p_admin_password);
$$;
...
begin
  ...
  return jsonb_build_object(
    ...
    'reservations', coalesce((select jsonb_agg(to_jsonb(x) order by x.date,x.start_minute) from (select id,date,start_minute,people,note,created_at,cancelled from reservations where member_id=m.id and cancelled=false) x),'[]'::jsonb),
    'booked_slots', coalesce((select jsonb_agg(jsonb_build_object('date',r.date,'start_minute',r.start_minute,'is_mine',r.member_id=m.id)) from reservations r where r.cancelled=false and r.date between current_date and current_date + 14),'[]'::jsonb),
    'closed_slots', coalesce((select jsonb_agg(to_jsonb(c)) from closed_slots c where c.date between current_date and current_date + 14),'[]'::jsonb),
    'purchases', coalesce((select jsonb_agg(to_jsonb(p) order by p.created_at desc) from slot_purchases p where p.member_id=m.id),'[]'::jsonb)
  );
end;
$$;
...
begin
  select * into m from fs_member_by_login(p_member_code,p_pin);
  if m.id is null then return jsonb_build_object('ok',false,'error','ログイン情報が違います。'); end if;
  insert into slot_purchases(member_id,month,price,slots,billing_status) values(m.id,mo,3000,1,'unconfirmed');
  return jsonb_build_object('ok',true);
end;
$$;
...
begin
  ...
  return jsonb_build_object(
    'ok', true,
    'members', coalesce((select jsonb_agg(to_jsonb(m) order by m.created_at desc) from members m where status='active'),'[]'::jsonb),
    'reservations', coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'member_id',r.member_id,'member_name',m.name,'member_code',m.member_code,'plan',m.plan,'date',r.date,'start_minute',r.start_minute,'people',r.people,'note',r.note,'created_by',r.created_by,'created_at',r.created_at,'cancelled',r.cancelled) order by r.date,r.start_minute) from reservations r join members m on m.id=r.member_id where r.cancelled=false),'[]'::jsonb),
    'closed_slots', coalesce((select jsonb_agg(to_jsonb(c) order by c.date,c.start_minute) from closed_slots c),'[]'::jsonb),
    'purchases', coalesce((select jsonb_agg(jsonb_build_object('id',p.id,'member_id',p.member_id,'member_name',m.name,'member_code',m.member_code,'plan',m.plan,'month',p.month,'price',p.price,'slots',p.slots,'billing_status',p.billing_status,'created_at',p.created_at,'billing_completed_at',p.billing_completed_at) order by p.created_at desc) from slot_purchases p join members m on m.id=p.member_id),'[]'::jsonb)
  );
end;
$$;
...
begin
  if not fs_is_admin(p_admin_password) then return jsonb_build_object('ok',false,'error','管理者パスコードが違います。'); end if;
  select 'FS'||lpad((coalesce(max(regexp_replace(member_code,'\D','','g')::int),0)+1)::text,3,'0') into code from members;
  pin := (floor(random()*9000)+1000)::int::text;
  insert into members(member_code,name,email,pin,plan) values(code,p_name,p_email,pin,p_plan) returning * into m;
...
begin
  if not fs_is_admin(p_admin_password) then return jsonb_build_object('ok',false,'error','管理者パスコードが違います。'); end if;
  update members set name=p_name,email=p_email,pin=p_pin,plan=p_plan where id=p_member_id;
  return jsonb_build_object('ok',true);
end;
$$;
...
begin
  if not fs_is_admin(p_admin_password) then return jsonb_build_object('ok',false,'error','管理者パスコードが違います。'); end if;
  update members set status='deleted' where id=p_member_id;
  update reservations set cancelled=true,cancelled_at=now() where member_id=p_member_id and cancelled=false;
  return jsonb_build_object('ok',true);
end;
$$;
...
begin
  if not fs_is_admin(p_admin_password) then return jsonb_build_object('ok',false,'error','管理者パスコードが違います。'); end if;
  select * into m from members where id=p_member_id and status='active';
  if m.id is null then return jsonb_build_object('ok',false,'error','会員が見つかりません。'); end if;
  if fs_slot_blocked(p_date,p_start_minute) or exists(select 1 from closed_slots where date=p_date and start_minute=p_start_minute) then return jsonb_build_object('ok',false,'error','この枠は予約できません。'); end if;
  insert into reservations(member_id,date,start_minute,people,note,created_by) values(m.id,p_date,p_start_minute,coalesce(nullif(p_people,''),'1名'),p_note,'admin');
  return jsonb_build_object('ok',true);
exception when unique_violation then
...
begin
  if not fs_is_admin(p_admin_password) then return jsonb_build_object('ok',false,'error','管理者パスコードが違います。'); end if;
  update reservations set cancelled=true,cancelled_at=now() where id=p_reservation_id;
  return jsonb_build_object('ok',true);
end;
$$;
...
begin
  ...
  if exists(select 1 from reservations where date=p_date and start_minute=p_start_minute and cancelled=false) then return jsonb_build_object('ok',false,'error','予約済みの枠です。'); end if;
  insert into closed_slots(date,start_minute,reason) values(p_date,p_start_minute,p_reason) on conflict(date,start_minute) do update set reason=excluded.reason;
  return jsonb_build_object('ok',true);
end;
$$;
...
begin
  if not fs_is_admin(p_admin_password) then return jsonb_build_object('ok',false,'error','管理者パスコードが違います。'); end if;
  delete from closed_slots where id=p_closed_slot_id;
  return jsonb_build_object('ok',true);
end;
$$;
...
begin
  if not fs_is_admin(p_admin_password) then return jsonb_build_object('ok',false,'error','管理者パスコードが違います。'); end if;
  update slot_purchases set billing_status='completed', billing_completed_at=now() where id=p_purchase_id;
  return jsonb_build_object('ok',true);
end;
$$;
...
begin
  if not fs_is_admin(p_admin_password) then return jsonb_build_object('ok',false,'error','管理者パスコードが違います。'); end if;
  insert into slot_purchases(member_id,month,price,slots,billing_status) values(p_member_id,to_char(current_date,'YYYY-MM'),0,1,'not_required');
  return jsonb_build_object('ok',true);
end;
$$;
