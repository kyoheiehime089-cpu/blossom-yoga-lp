(function(){
  ...
  function ensureTab(){
    ...
    const anchor = $('#appsTab') || $('#settingsTab') || $('#requestsTab') || $('#customersTab') || $('#calendarTab') || $('#dashboardTab');
    if(tabs) tabs.insertAdjacentHTML('beforeend', '<button class="tab" data-tab="condition">コンディション管理</button>');
    if(anchor) anchor.insertAdjacentHTML('afterend', '<section id="conditionTab" class="panel hidden admin-condition-panel"><p class="eyebrow">コンディション管理</p><h2>コンディション管理</h2><div id="adminDailyCheckContent" class="list admin-daily-check-list"><article class="res admin-daily-card"><p>読み込み中...</p></article></div></section>');
  }
