(()=>{
  ...
  function toast(msg){
    if(window.adminSoftToast){window.adminSoftToast(msg);return;}
    alert(msg);
  }
  ...
  async function openConfirm(btn){
    ...
    const pass=document.getElementById('adminPass')?.value||sessionStorage.getItem('fs_admin_pass')||'';
    ...
    if(!r){toast('予約が見つかりません。表示を更新してください。');return;}
    target={id,pass,button:btn};
  ...
  function removeCancelledFromScreen(id){
    document.querySelectorAll(`[data-cancel="${id}"]`).forEach(btn=>{
      const row=btn.closest('.slot-row')||btn.closest('article.res')||btn.closest('article');
      if(row){
  ...
  async function doCancel(){
    ...
    removeCancelledFromScreen(target.id);
    toast('予約をキャンセルしました');
    target=null;
  }
