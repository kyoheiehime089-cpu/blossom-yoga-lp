-- friendsセルフ game-lab コンテンツハブ v_app_hub_02_content_hub
...
alter table public.fs_member_apps
  add column if not exists category_label text,
  add column if not exists title text,
  add column if not exists description text,
  add column if not exists content_type text not null default 'game',
  add column if not exists href text;

update public.fs_member_apps
set
  category_label = coalesce(category_label, category_name),
  title = coalesce(title, app_name),
  description = coalesce(description, ''),
  href = coalesce(href, app_url),
  content_type = coalesce(content_type, 'game');

alter table public.fs_member_apps
  alter column category_label set default '',
  alter column title set default '',
  alter column description set default '',
  alter column href set default './stress-game.html';
...
insert into public.fs_member_apps (
  app_key,
  category_key,
  category_name,
  category_label,
...
values (
  ...
  'ブロックパズル',
  'ブロックパズル',
  '1日1回、3分だけできるストレスリセット用のブロックパズルです。',
  'game',
  './stress-game.html',
  './stress-game.html',
  true,
  ...
  return jsonb_build_object(
    ...
    'apps', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'category_label', coalesce(nullif(category_label, ''), category_name),
        'app_name', app_name,
        'title', coalesce(nullif(title, ''), app_name),
        'description', coalesce(description, ''),
        'content_type', coalesce(content_type, 'game'),
        'app_url', app_url,
        'href', coalesce(nullif(href, ''), app_url),
        'enabled', enabled,
  ...
  return jsonb_build_object(
    ...
    'logs', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'app_key', a.app_key,
        'app_name', a.app_name,
        'title', coalesce(nullif(a.title, ''), a.app_name),
        'category_key', a.category_key,
        'category_name', a.category_name,
        'category_label', coalesce(nullif(a.category_label, ''), a.category_name),
        'content_type', coalesce(a.content_type, 'game'),
...
begin
  ...
  if v_rows = 0 then
    return jsonb_build_object('ok', false, 'error', 'コンテンツが見つかりません。');
  end if;
